...
Page({
    ...
    _loadOrderData: function() {
        ...
        var par = {
            ...
            callback: function(data) {
                data.otime = order.formatTimeTwo(data.otime, 'Y-M-D h:m:s')
                    //有没有随机立减
                if (data.is_reduce == 1) {
    ...
    tobox: function(event) {
        var jumptype = order.getDataSet(event, 'jumptype')
        var s_id = order.getDataSet(event, 's_id')
        if (jumptype == 1) {
    ...
    chooseway: function(event) {
        var paytype = order.getDataSet(event, 'id') //支付类型
