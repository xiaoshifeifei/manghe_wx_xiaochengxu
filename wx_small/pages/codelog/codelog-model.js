...
class CodeLog extends Base{
    ...
    verify(scenecode,callback) {
        var token = wx.getStorageSync('token');
        if (!token) {
            this.getTokenFromServer(scenecode,(data)=>{
                callback(data)
            });
        } else {
            this._veirfyFromServer(token,scenecode,(data)=>{
                callback(data)
            });
        }
    }
    ...
    /*验证token是否过期*/
    _veirfyFromServer(token,scenecode,callback) {
        var that = this;
        wx.request({
            url: that.verifyUrl,
            method: 'POST',
            ...
            success: function (res) {
                ...
                if(!valid){
                    that.getTokenFromServer(scenecode,(data)=>{
                        callback(data)
                    });
                }else{
                    callback(res.data)
    ...
    /*获取token*/
    getTokenFromServer(scenecode,callBack) {
        var that  = this;
        wx.login({
            success: function (res) {
                wx.request({
